...
class Mixer_additive(qs.SanUnit):
    """
    Dummy mixer to calculate additive mass flow as function of the plant capacity. 
    
    Parameters
    ----------
    ins : 
        Inlet solvent, (the default is PEG).
    outs : 
    ...
    # compute the solvent mass (kg/h)
        additive_mass_kg_h = (p.v_polymersol * p.d_polymersol * p.additive_fraction) / 1000.0 / 24.0
    ...
    # obtain inlet stream object safely (handles N_ins == 1 or >1)
        ...
        inlet.imass['PEG'] = additive_mass_kg_h
