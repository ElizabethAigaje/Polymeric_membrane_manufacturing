...
class Mixer_glycerol_conditioning(qs.SanUnit):
    """     
    Dummy mixer to calculate glycerol for conditioning (prior drying) as function of the plant capacity. 
    ...
    ins : 
        Inlet of glyerol.
    outs : 
        Outlet of glycerol for conditioning.
    fraction_filled :
        It is the fraction of pore volume that is filled during the conditioning process, (the default is 1.1)
    params:
    ...
    Rinsign water demand was assumed to be the same as non-solvent demand.
    The glycerol solution requirement was calculated based on the membrane
    pore volume assumed to be filled during conditioning. We assume 10% extra, as 
    according to the industry members you never work in the lower limit.
    ...
    def _run(self):
        ...
        conditioning_solution_mass = conditioning_solution_volume*p.d_conditioningsol  #kg conditioning solution / polymer fiber
        conditioning_glycerol_needed = conditioning_solution_mass*(p.conditioning_concentration/p.polymermass_per_fiber)*polymer_needed  #kg glycerol/h
    ...
    # obtain inlet stream object safely (handles N_ins == 1 or >1)
        ...
        inlet.imass['glycerol'] = conditioning_glycerol_needed
